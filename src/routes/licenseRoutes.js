      ...
      license.save((error) => {
        if (error) {
          ...
          return res.status(500).send({
            status: 'error',
            msg: 'Error: Solicitud de permiso no generada',
          });
        }
        res
          .status(200)
          .send({ status: 'ok', msg: 'Solicitud de permiso guardada' });
      });
    }
  });
});
    ...
    User.populate(license, { path: 'id_usuario' }, function (error, license) {
      ...
      if (license) {
        const licenseList = license.map((l) => ({
          ...
          id_permisos: l._id,
          fecha_inicio: `${l.fecha_inicio.getFullYear()}-${
            l.fecha_inicio.getMonth() + 1 < 10
              ? '0' + (l.fecha_inicio.getMonth() + 1)
              : l.fecha_inicio.getMonth() + 1
          }-${
            l.fecha_inicio.getDate() < 10
              ? '0' + l.fecha_inicio.getDate()
              : l.fecha_inicio.getDate()
          }`,
          fecha_fin: `${l.fecha_fin.getFullYear()}-${
            l.fecha_fin.getMonth() + 1 < 10
              ? '0' + (l.fecha_fin.getMonth() + 1)
              : l.fecha_fin.getMonth() + 1
          }-${
            l.fecha_fin.getDate() < 10
              ? '0' + l.fecha_fin.getDate()
              : l.fecha_fin.getDate()
          }`,
        }));
        res.send({
          estado: 'ok',
          msg: 'Permiso encontrado',
          data: licenseList,
        });
      }
      if (!license) {
...
licenseRoutes.post('/license_edit', (req, res) => {
  const { id_permisos, estado, comentarios } = req.body;
  licenseModel.updateOne(
    { _id: id_permisos },
