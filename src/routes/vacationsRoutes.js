          ...
          if (user) {
            vacations.save((error) => {
              if (error) {
                ...
                return res
                  .status(500)
                  .send({ status: 'error', msg: 'Error: Solicitud de Vacación no generada' });
              }
              res.status(200).send({ status: 'ok', msg: 'Solicitud de vacación guardada' });
            });
          }
...
vacationsRoutes.get("/vacations_list", (req, res) => {
    vacationsModel.find({}, (error, vacations) => {
        User.populate(vacations, {path:"id_usuario"}, function(error, vacations){
            if (error) {
                console.log(error);
            ...
            if (vacations) {
                ...
                res.send({ estado: 'ok', msg: 'Vacaciones encontradas', data: vacationsList });
            }
            if (!vacations) {
...
vacationsRoutes.get("/vacations_search/:id_usuario/:_id", (req, res) => {
    ...
    vacationsModel.findOne({$and:[{id_usuario}, {_id}]}, (error, vacations) => {
        User.populate(vacations, {path:"id_usuario"}, function(error, vacations){
            if (error) {
                console.log(error);
                return res.send({ estado: 'error', msg: 'Error al buscar' });
...
vacationsRoutes.post("/vacations_edit", (req, res) => {
    ...
    vacationsModel.updateOne({_id:id_vacaciones}, {$set:{estado:estado}}, (error, info) => {
        if (error) {
            console.log(error);
            return res.send({ estado: 'error', msg: 'Error al buscar' });
        }
